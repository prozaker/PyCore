...
# use this if u want to send the obj change to everyone in arena
PID_ALL = 0xFFFF
...
class LVZMapObject:
    """A class that holds an LVZ Map object. All variables are private,
    as modifying any single value results in another value being
    changed.

    All variables can be accessed through getVariable() methods.

    When initalizing, pass all values as they are in your compressed
    LVZ file, use the global variables listed above (MODE, LAYER).

    The variable 'time' is the displaytime as defined in LVZ file. The
    variable 'bot' should be the SubspaceBot object your bot runs off
    of.

    After doing the changes you wish to do with setVariable(). When you
    have changed all the values you wish, use doUpdate() to send the
    changes to the server.
    """

    # devs do not need to know the details of how this packet works.
    def __init__(self, bot, pid, objectID, x_pos, y_pos, image_id, layer,
                 time, mode):

        if not isinstance(bot, SubspaceBot):
            raise Exception(
                'the variable Bot should be of the class SubspaceBot'
            )
