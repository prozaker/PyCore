...
class PlayerInfoBase:
    def __init__(self):
        self.__active = 0
        # if here  = 0 then time stamp is when he left otherwise it is when
        # he entered
...
class PlayerInfoManager():
    def __init__(
            self, initializer, cachetime=6*60*60, persist=1, return_new=1):
        self.pinfo_dict = {}
    ...
    def GetPlayerInfo(self, name):
        ...
        if the playerinfo manager is initialized with
        return_new = 0/false it will return None when a playerinfo
        isnt found so in that case you will have to make your own
        and insetit using setplayerinfo
        """
    ...
    def DeleteExpired(self):
        """
        use this in event timer or tick to delete all inactives after
        cachetime
        """
    ...
    def HandleEvents(self, ssbot, event):
        ...
        this at the top of your bots handleEvents function if it is
        more convenient this will handle all of the setting/deleting
        etc etc
        """
