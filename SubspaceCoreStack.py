...
# for sending a high priority packet, like position updates or ack messages
PRIORITY_HIGH = 0
# for sending normal priority packets
PRIORITY_NORMAL = 1
...
# how often EVENT_TICK happens, in hs, backed by an accumulator
EVENT_TICK_INTERVAL = 10
# not backed by an accumulator
EVENT_INTERNAL_CORE_PERIODIC_INTERVAL = 10

# after 15 seconds without receiving data, disconnect
NO_PACKET_RECEIVED_TIMEOUT_INTERVAL = 1500
...
class QueuedPacket:
    ...
    def __init__(self, data, reliable=False):
        ...
        if self.totalPacketSize() > MAX_PACKET:
            raise Exception(
                "Packet has a size greater than the maximum allowed: %d" %
                self.totalPacketSize()
            )

    def totalPacketSize(self):
        """Compute the total packed size, including the size of the
        reliable header if necessary."""
        if self.reliable:
...
class CoreStack:
    ...
    def __init__(self, debug=False, logger=None):
        ...
        self.__packet_handlers = {
            ...
            0x0D: self.__handleDisconnectPacket,
            ...
            try:
                ...
                if game_type == 0x00:
                    if core_type == 0x05:    # response to __sendClientKey()
                        ...
                        self.__crypto = SubspaceEncryption.SubspaceEncryption(
        ...
        if clusterable_packets > 1:
            ...
            for packet_list in [h, n]:
                ...
                for p in packet_list[:]:
                    ...
                    if p.totalPacketSize() <= 255 and \
                            len(data) + p.totalPacketSize() + 1 <= MAX_PACKET:
                        if p.reliable:
            ...
            if p.reliable:
                ...
                self.__reliable_messages_in_transit[
                    self.__next_outgoing_ack_id] = [data, GetTickCountHs()]
                self.__next_outgoing_ack_id += 1
    ...
    def waitForEvent(self):
        ...
        while self.__connected:
            ...
            if TickDiff(now, self.__last_core_periodic_event_tick) > \
...
#            return
        ...
        if self.__crypto:
            try:
                ...
                if type == 0x00:
                    begin_offset = 2
                packet = packet[:begin_offset] + self.__crypto.decryptData(
                    packet[begin_offset:])
    ...
    def __queueSyncResponse(self):
        self.queuePacket(
            struct.pack(
                ...
                GetTickCountHs(),
                TickDiff(
                    GetTickCountHs(), self.__last_sync_response_received_tick)
            ), priority=PRIORITY_HIGH
        )
    ...
    def __sendRawPacket(self, packet):
        ...
        if self.__crypto:
            ...
            packet = packet[:begin_offset] + self.__crypto.encryptData(
                packet[begin_offset:])
